...
def find_events(
    ...
    """
    print(f"\n    Debug: Starting calendar event fetch for {course_code}")
    print(f"    Debug: Using base URL: {canvas_base_url}")
    ...
    try:
        ...
        print(f"    Debug: Found {len(events if isinstance(events, list) else events.get('calendar_events', []))} events")
