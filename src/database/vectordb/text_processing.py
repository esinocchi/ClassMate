        ...
        if doc.get(source_field):
            try:
                ...
                utc_dt = datetime.fromisoformat(
                    doc[source_field].replace("Z", "+00:00")
                )
                ...
                print(
                    f"Warning: Could not parse or convert time for {source_field} in doc {doc.get('id')}: {e}"
                )
                doc[target_field] = None  # Or keep original, or add error string


def preprocess_text_for_embedding(
    doc: Dict[str, Any], course_dict: Dict[str, str]
) -> str:
    """
    ...
    try:
        ...
        if not natural_language_output or natural_language_output.strip() == "passage:":
            print(
                f"Warning: to_natural_language produced empty output for doc ID: {modified_doc.get('id')}"
            )
        ...
        print(
            f"Error during to_natural_language conversion for doc ID {modified_doc.get('id')}: {e}"
        )
        return ""
...
def to_natural_language(item: Dict[str, Any]) -> str:
    ...
    templates = {
        "quiz": [
            ...
            (
                "allowed_attempts",
                lambda v, t=title: f"{t} allows unlimited attempts"
                if v == -1
                else f"{t} allows {v} attempts",
            ),
            ("local_due_time", f"{title} is due on {{}}"),
            (
                "locked_for_user",
                lambda v, t=title: f"{t} is locked" if v else f"{t} is available",
            ),
        ],
        "assignment": [
            ("local_due_time", f"{title} is due on {{}}"),
            (
                "submission_types",
                lambda v, t=title: f"{t} accepts submission types: {{', '.join(v)}}"
                if isinstance(v, list)
                else f"{t} accepts submission types: {v}",
            ),
            (
                "can_submit",
                lambda v, t=title: f"{t} can be submitted"
                if v
                else f"{t} cannot be submitted",
            ),
        ],
        "announcement": [
        ...
        if item.get(source_field):
            try:
                date_obj = datetime.fromisoformat(
                    item[source_field].replace("Z", "+00:00")
                )
