...
class CanvasBM25:
    ...
    def _tokenize(self, text: str) -> List[str]:
        ...
        return list(
            filter(
                lambda token: len(token) > 2 or re.match(r"^[a-z]+\d+$", token), tokens
            )
        )
    ...
    def _precompute(self):
        ...
        self.avg_doc_length = (
            total_length / len(self.documents) if self.documents else 0
        )
            ...
            if term_frequency_in_doc > 0:
                ...
                if documents_containing_term > 0:
                    score += self._calculate_bm25_score(
                        term_frequency_in_doc, documents_containing_term, doc_length
                    )
    ...
    def _calculate_bm25_score(
        ...
        inverse_document_frequency = math.log(
            (len(self.documents) - documents_containing_term + 0.5)
            / (documents_containing_term + 0.5)
        )
        length_norm = (1 - self.b) + self.b * (doc_length / self.avg_doc_length)
        return (
            max(inverse_document_frequency, 0)
            * (term_frequency_in_doc * (self.k1 + 1))
            / (term_frequency_in_doc + self.k1 * length_norm)
        )
