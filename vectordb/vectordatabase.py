...
class VectorDatabase:
    ...
    def search(self, query: str, course_ids: Optional[List[str]] = None, top_k: int = 5,
        ...
        logger.debug(f"Course IDs filter: {course_ids}")
        ...
        try:
            results = self.collection.query(
                ...
                n_results=top_k * 3,  # Get more results than needed for post-processing
                where=where_clause, # Apply filters if any
                include=["distances", "documents"]
            )
        except Exception as e:
            logger.error(f"ChromaDB query error: {e}")
            # If the query fails (possibly due to complex filter), try a simpler approach
            try:
        ...
        # Apply post-query course filtering if we couldn't use ChromaDB's filtering
        if course_ids and where_clause is None:
            logger.info("Applying manual course filtering after query")
        ...
        for i, doc_id in enumerate(doc_ids):
            ...
            # Manual course filtering if ChromaDB filtering didn't work
            if course_ids and where_clause is None:
                doc_course_id = str(doc.get('course_id', ''))
                if doc_course_id not in course_ids:
                    continue
        ...
        if include_related and search_results:
            ...
            for doc in related_docs:
                # Apply course filtering to related documents too
                if course_ids:
                    doc_course_id = str(doc.get('course_id', ''))
                    if doc_course_id not in course_ids:
                        continue
        ...
        # Post-process to prioritize exact filename/display name matches
        exact_matches = []
        partial_matches = []
        other_results = []
        ...
        for result in search_results:
            ...
            # Get the document name based on its type
            if doc_type == 'file':
            ...
            # Check for exact match with the query
            if doc_name == normalized_query.lower():
                # Significant boost for exact matches
                result['similarity'] += 0.5
                exact_matches.append(result)
            # Check for partial matches (query terms in the document name)
...
if __name__ == "__main__":
    ...
    results = vector_db.search("HW2 HW2 assignment CMPSC465 Spring 2025 Section 1/2", top_k=10, course_ids=["2361957"])
    ...
    if not results:
        print("No results found matching the query with the specified course ID filter.")